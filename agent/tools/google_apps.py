...
class Gmail:
    ...
    def list_unread(self) -> list[Message]:
        ...
        res = self.svc.users().messages().list(userId="me", q=q, maxResults=20).execute()
        out = []
    ...
    def mark_processed(self, msg_id: str) -> None:
        self.svc.users().messages().modify(userId="me", id=msg_id,
                                           body={"addLabelIds": [self._done_id], "removeLabelIds": ["UNREAD"]}).execute()
...
class Drive:
    ...
    def _find(self, path: str) -> Optional[dict]:
        ...
        for part in path.split("/"):
            ...
            res = self.svc.files().list(q=q, fields="files(id,name,mimeType)", pageSize=1).execute().get("files", [])
            if not res:
    ...
    def _ensure_folder(self, parts: list[str]) -> str:
        ...
        for part in parts:
            ...
            parent = res[0]["id"] if res else self.svc.files().create(
                body={"name": part, "mimeType": "application/vnd.google-apps.folder", "parents": [parent]}, fields="id").execute()["id"]
        return parent
    ...
    def write_text(self, path: str, content: str, idempotency_key: str = ""):
        ...
        *folders, name = path.split("/")
        ...
        if existing:
            f = self.svc.files().update(fileId=existing["id"], media_body=media, fields="id,webViewLink").execute()
            return {"id": f["id"], "link": f.get("webViewLink"), "deduped": True}
        f = self.svc.files().create(body={"name": name, "parents": [parent]}, media_body=media, fields="id,webViewLink").execute()
    ...
    def read_text(self, path: str) -> str:
        ...
        if f["mimeType"] == "application/vnd.google-apps.document":
            return self.svc.files().export(fileId=f["id"], mimeType="text/plain").execute().decode()
        buf = io.BytesIO()
        dl = MediaIoBaseDownload(buf, self.svc.files().get_media(fileId=f["id"]))
        done = False
        while not done:
...
class Sheets:
    ...
    def _rows(self) -> list[list[str]]:
        res = self.svc.spreadsheets().values().get(spreadsheetId=self.sheet_id, range=f"{self.tab}!A1:N1000").execute()
...
class GCalendar:
    ...
    def create_event(self, title, day, description, idempotency_key):
        ...
        existing = self.svc.events().list(calendarId=self.cal, privateExtendedProperty=f"key={idempotency_key}",
                                          maxResults=1).execute().get("items", [])
        if existing:
        ...
        ev = self.svc.events().insert(calendarId=self.cal, body=body).execute()
